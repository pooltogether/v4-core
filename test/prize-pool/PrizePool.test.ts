  ...
  describe('Core Functions', () => {
    describe('award()', () => {
      it('should return early if amount is 0', async () => {
        ...
        await expect(prizePool.award(wallet2.address, toWei('0')))
          .to.not.emit(prizePool, 'Awarded')
      });
      ...
      it('should fail if amount is GREATER THEN the current award balance', async () => {
        ...
        await expect(prizePool.award(wallet2.address, toWei('2000')))
          .to.be.revertedWith('PrizePool/award-exceeds-avail')
      });
  ...
  describe('Getter Functions', () => {
    ...
    describe('isControlled()', () => {
      it('should validate TRUE with ticket variable', async () => {
        expect(await prizePool.isControlled(await prizePool.getTicket()))
          .to.equal(true);
      });

      it('should validate FALSE with non-ticket variable', async () => {
        expect(await prizePool.isControlled(AddressZero))
          .to.equal(false);
      });
    });
