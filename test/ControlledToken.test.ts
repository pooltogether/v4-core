...
describe('ControlledToken', () => {
  ...
  beforeEach(async () => {
    ...
    if (!isConstructorTest) {
      await deployToken();
      token2 = token.connect(wallet2 as Signer);
    }
  });
  ...
  describe('controllerMint()', () => {
    ...
    it('should only be callable by the controller', async () => {
      ...
      await expect(
        token.controllerMint(wallet.address, amount),
  ...
  describe('controllerBurn()', () => {
    ...
    it('should only be callable by the controller', async () => {
      ...
      await expect(
        token.controllerBurn(wallet.address, amount),
      ).to.be.revertedWith('ControlledToken/only-controller');
    });
  });
  ...
  describe('controllerBurnFrom()', () => {
    ...
    it('should not allow non-approved users to burn', async () => {
      ...
      await expect(
        controller.call(token, 'controllerBurnFrom', wallet2.address, wallet.address, amount),
      ).to.be.revertedWith('');
    });
