...
describe('PeriodicPrizeStrategy', () => {
  ...
  beforeEach(async () => {
    ...
    TokenListenerInterface = await hre.artifacts.readArtifact("contracts/import/token/TokenListenerInterface.sol:TokenListenerInterface")
    ...
    const PrizePool = await hre.artifacts.readArtifact("contracts/import/prize-pool/PrizePool.sol:PrizePool")
    ...
    const ControlledToken = await hre.artifacts.readArtifact("contracts/import/token/ControlledToken.sol:ControlledToken")
    ...
    const PeriodicPrizeStrategyListenerInterface = await hre.artifacts.readArtifact("contracts/import/prize-strategy/PeriodicPrizeStrategyListenerInterface.sol:PeriodicPrizeStrategyListenerInterface")
    ...
    const PeriodicPrizeStrategyHarness =  await hre.ethers.getContractFactory("ClaimableDrawPrizeStrategyHarness", wallet, overrides)
  ...
  describe('setRngService', () => {
    ...
    it('should not allow anyone but the owner to change', async () => {
      ...
      await expect(prizeStrategyWallet2.setRngService(token.address)).to.be.revertedWith('Ownable: caller is not the owner')
    })
  ...
  describe('addExternalErc20Award()', () => {
    ...
    it('should allow the listeners to add external erc20s', async () => {
      ...
      const BeforeAwardListener = await hre.artifacts.readArtifact("contracts/import/prize-strategy/BeforeAwardListener.sol:BeforeAwardListener")
  ...
  describe('setClaimableDraw()', () => {
    it('should fail to set draw manager from unauthorized wallet', async () => {
      ...
      await expect(prizeStrategyUnauthorized.setClaimableDraw(wallet2.address))
        .to.be.revertedWith('Ownable: caller is not the owner')
    })

    it('should fail to set draw manager with zero address', async () => {
      await expect(prizeStrategy.setClaimableDraw(constants.AddressZero))
        .to.be.revertedWith('ClaimableDraw/claimable-draw-not-zero-address')
    })
    
    it('should fail to set draw manager with existing draw manager', async () => {
      await expect(prizeStrategy.setClaimableDraw(claimableDraw.address))
        .to.be.revertedWith('ClaimableDraw/existing-claimable-draw-address')
    })
  ...
  describe('claim()', () => {
    ...
    it.skip('should succeed to claim award', async () => {
      ...
      await prizePool.mock.award
        .withArgs(wallet.address.toLowerCase(), MOCK_DRAW.payout, ticket.address.toLowerCase())
