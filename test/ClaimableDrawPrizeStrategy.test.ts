// @ts-nocheck
import { deployMockContract, deployContract } from 'ethereum-waffle';
...
describe('ClaimableDrawPrizeStrategy', () => {
  ...
  let drawCalculator, claimableDraw;
  ...
  beforeEach(async () => {
    ...
    const PeriodicPrizeStrategyDistributorInterface = await hre.artifacts.readArtifact(
      'PeriodicPrizeStrategyDistributorInterface',
    );
    distributor = await deployMockContract(
      wallet,
      PeriodicPrizeStrategyDistributorInterface.abi,
      overrides,
    );
    ...
    const PeriodicPrizeStrategyHarness = await hre.ethers.getContractFactory(
    ...
    debug('deploying claimableDraw...');
    let IDrawCalculator = await artifacts.readArtifact('IDrawCalculator');
    drawCalculator = await deployMockContract(wallet, IDrawCalculator.abi);
  ...
  describe('initialize()', () => {
    it('should emit an Initialized event', async () => {
      ...
      const initalizeResult2 = prizeStrategy2.initialize(
        prizePeriodStart,
        prizePeriodSeconds,
        prizePool.address,
        ticket.address,
      ...
      await expect(initalizeResult2)
        .to.emit(prizeStrategy2, 'Initialized')
        .withArgs(
          prizePeriodStart,
          prizePeriodSeconds,
          prizePool.address,
    ...
    it('should reject invalid params', async () => {
      ...
      initArgs[1] = 0;
      await expect(prizeStrategy2.initialize(...initArgs)).to.be.revertedWith(
      ...
      initArgs = _initArgs.slice();
      initArgs[4] = AddressZero;
      await expect(prizeStrategy2.initialize(...initArgs)).to.be.revertedWith(
        'PeriodicPrizeStrategy/sponsorship-not-zero',
      );
      initArgs = _initArgs.slice();
      initArgs[5] = AddressZero;
      await expect(prizeStrategy2.initialize(...initArgs)).to.be.revertedWith(
  ...
  describe('setRngService', () => {
    ...
    it('should not allow anyone but the owner to change', async () => {
      const prizeStrategyWallet2 = prizeStrategy.connect(wallet2);
      await expect(prizeStrategyWallet2.setRngService(token.address)).to.be.revertedWith(
    ...
    it('should not be called if an rng request is in flight', async () => {
      ...
      await expect(prizeStrategy.setRngService(token.address)).to.be.revertedWith(
  ...
  describe('cancelAward()', () => {
    it('should not allow anyone to cancel if the rng has not timed out', async () => {
      await expect(prizeStrategy.cancelAward()).to.be.revertedWith(
        'PeriodicPrizeStrategy/rng-not-timedout',
      );
    });
    ...
    it('should allow anyone to reset the rng if it times out', async () => {
      ...
      await prizeStrategy.setCurrentTime(
        (await prizeStrategy.prizePeriodEndAt())
          .add(await prizeStrategy.rngRequestTimeout())
          .add(1),
      );
  ...
  describe('setBeforeAwardListener()', () => {
    ...
    it('should not allow anyone else to set it', async () => {
      await expect(
        prizeStrategy.connect(wallet2).setBeforeAwardListener(beforeAwardListener.address),
      ).to.be.revertedWith('Ownable: caller is not the owner');
    });

    it('should not allow setting an EOA as a listener', async () => {
      await expect(prizeStrategy.setBeforeAwardListener(wallet2.address)).to.be.revertedWith(
  ...
  describe('setPeriodicPrizeStrategyListener()', () => {
    it('should allow the owner to change the listener', async () => {
      await expect(
        prizeStrategy.setPeriodicPrizeStrategyListener(periodicPrizeStrategyListener.address),
      )
    ...
    it('should not allow setting an EOA as a listener', async () => {
      await expect(
        prizeStrategy.setPeriodicPrizeStrategyListener(wallet2.address),
  ...
  describe('setTokenListener()', () => {
    ...
    it('should not allow anyone else to change the listener', async () => {
      await expect(
        prizeStrategy.connect(wallet2).setTokenListener(tokenListener.address),
      ).to.be.revertedWith('Ownable: caller is not the owner');
    });

    it('should not allow setting an EOA as a listener', async () => {
      await expect(prizeStrategy.setTokenListener(wallet2.address)).to.be.revertedWith(
    ...
    it('should award the winner', async () => {
      ...
      await periodicPrizeStrategyListener.mock.afterPrizePoolAwarded
        .withArgs(
          ...
          await prizeStrategy.prizePeriodStartedAt(),
        )
        .returns();
      ...
      expect(await prizeStrategy.prizePeriodStartedAt()).to.equal(
  ...
  describe('claim()', () => {
    ...
    beforeEach(async () => {
      let ClaimableDraw = await artifacts.readArtifact('ClaimableDraw');
      claimableDraw = await deployMockContract(wallet, ClaimableDraw.abi);
  ...
  describe('_distributePrizeSplits()', () => {
    it('should award prize splits in _distribute', async () => {
      ...
      await prizePool.mock.award
        .withArgs(wallet4.address, toWei('12'), sponsorship.address)
        .returns();
  ...
  describe('calculateNextPrizePeriodStartTime()', () => {
    it('should always sync to the last period start time', async () => {
      let startedAt = await prizeStrategy.prizePeriodStartedAt();
      expect(
        await prizeStrategy.calculateNextPrizePeriodStartTime(
  ...
  describe('setPrizePeriodSeconds()', () => {
    ...
    it('should not allow non-owners to set the prize period', async () => {
      await expect(prizeStrategy.connect(wallet2).setPrizePeriodSeconds(99)).to.be.revertedWith(
        'Ownable: caller is not the owner',
      );
    });
  });
  ...
  describe('with a prize-period scheduled in the future', () => {
    ...
    describe('startAward()', () => {
      it('should prevent starting an award', async () => {
        ...
        await expect(prizeStrategy2.startAward()).to.be.revertedWith(
