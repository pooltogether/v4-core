...
module.exports = async (hardhat) => {
  ...
  if (isTestEnvironment && !harnessDisabled) {
    compoundPrizePoolProxyFactoryResult = await deploy("CompoundPrizePoolProxyFactory", {
      ...
      contract: 'CompoundPrizePoolProxyFactory',
      from: deployer,
      skipIfAlreadyDeployed: true
    })
  } else {
  ...
  const controlledTokenProxyFactoryResult = await deploy("ControlledTokenProxyFactory", {
    contract: 'ControlledTokenProxyFactory2',
    from: deployer,
    skipIfAlreadyDeployed: true
  })
  ...
  if (isTestEnvironment && !harnessDisabled) {
    ...
    tsunamiDrawCalculatorProxyFactory = await deploy("TsunamiDrawCalculatorProxyFactory", {
      contract: 'TsunamiDrawCalculatorProxyFactory',
      from: deployer,
      skipIfAlreadyDeployed: true
    })
  }
  ...
  if (isTestEnvironment && !harnessDisabled) {
    ...
    claimableDrawProxyFactoryResult = await deploy("ClaimableDrawProxyFactory", {
      contract: 'ClaimableDrawProxyFactory',
      from: deployer,
      skipIfAlreadyDeployed: true
    })
  }
  ...
  if (isTestEnvironment && !harnessDisabled) {
    claimableDrawPrizeStrategyProxyFactoryResult = await deploy("ClaimableDrawPrizeStrategyProxyFactory", {
      contract: 'ClaimableDrawPrizeStrategyProxyFactory',
      from: deployer,
      skipIfAlreadyDeployed: true
    })
  } else {
  ...
  displayResult('ClaimableDrawPrizeStrategyProxyFactory', claimableDrawPrizeStrategyProxyFactoryResult)
  ...
  const claimableDrawPrizeStrategtBuilderResult = await deploy("ClaimableDrawPrizeStrategyBuilder", {
    args: [
  ...
  const poolClaimableDrawPrizeStrategyBuilder = await deploy("PoolClaimableDrawPrizeStrategyBuilder", {
    args: [
      reserveRegistry,
