...
contract DrawCalculatorHarness is DrawCalculator {
  ...
  function createBitMasks(DrawLib.PrizeDistribution calldata _drawSettings) public pure returns (uint256[] memory) {
  ...
  function calculatePrizeDistributionFraction(DrawLib.PrizeDistribution calldata _drawSettings, uint256 _prizeDistributionIndex) external pure returns (uint256)
  {
    return _calculatePrizeDistributionFraction(_drawSettings, _prizeDistributionIndex);
  }

  function numberOfPrizesForIndex(uint8 _bitRangeSize, uint256 _prizeDistributionIndex) external view returns (uint256) {
    return _numberOfPrizesForIndex(_bitRangeSize, _prizeDistributionIndex);
  }

  function calculateNumberOfUserPicks(DrawLib.PrizeDistribution memory _drawSettings, uint256 _normalizedUserBalance) external pure returns (uint256){
